    ...
    if(modofunc == 1) {
      ...
      while(modofunc == 1) {
         ...
         if(tempoliga != 0) {
            ...
            if (digitalRead(BOMBA)) { // Verifica o ESTADO ATUAL do pino (HIGH/LOW)
                ...
                if (tempoatual - tempoanterior >= tempoliga) {
                    ...
                    Serial.println("Bomba desligada pelo ciclo");
                    Serial.print("tempo informado para o ciclo desliga: ");
                ...
                if (tempoatual - tempoanterior >= tempodesliga) {
                    ...
                    tempoanterior = tempoatual;  // Reinicia o contador
                    Serial.println("Bomba ligada pelo ciclo");
                    Serial.print("tempo informado para o ciclo liga: ");
            ...
            Serial.println("Bomba ligada continuamente (até sair do modo automático)");
            ThingSpeak.setField(3, 1);  //atualiza thingspeak
            ThingSpeak.writeFields(Channel, writeApiKey);
        }
        ...
        if(temperatura < settemp){
          ...
          Serial.println("Aquecedor ligado (controle automático buscando setpoint)");
          ...
          Serial.println("Aquecedor desligado (controle automático)");
      ...
      Serial.println("Saindo do modo de funcionamento automático - deligando bomba e aquecedor local e no thingspeak");
...
//------------------------------------------------------------------------------------------------------------------
      ...
      if(est_bba == 1){
        ...
        Serial.println("Bomba desligada - modo manual");
      }
      ...
      if(est_aqu == 1){
        digitalWrite(AQUECEDOR, HIGH);
        Serial.println("Aquecedor ligado - modo manual");
      }
      else if(est_aqu == 0){
        digitalWrite(AQUECEDOR, LOW);
        Serial.println("Aquecedor desligado - modo manual");
      }
